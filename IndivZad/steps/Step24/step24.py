...
class CityObject:
    ...
    # Виртуальный метод для получения значения атрибута.
        if hasattr(self, attribute_name):
            return getattr(self, attribute_name)
        else:
            raise AttributeError(f"Объект {self.__class__.__name__} не имеет атрибута '{attribute_name}'.")

    def set_attribute(self, attribute_name, value):
    # Виртуальный метод для установки значения атрибута.
        if hasattr(self, attribute_name):
...
class Park(CityObject):
    def __init__(self, name=None, objType="Park", capacity=0, owner=None, openingDate=None, greenArea=0, **kwargs):
        super().__init__(name, objType, capacity, owner, openingDate, **kwargs)
        self.greenArea = greenArea


    def get_attribute(self, attribute_name):
        if attribute_name == "greenArea":
            return self.greenArea
        else:
            return super().get_attribute(attribute_name)


    def addMaintenanceEvent(self, eventDate, eventName, eventType, visitors=0, maintenanceType=None):
        # New method specific to parks
        old_events = self.events.copy()
        self.events.append(Event(eventDate, eventName, eventType, visitors))
        new_events = self.events
        self.transactions.append((datetime.utcnow(), "addMaintenanceEvent", "events", old_events, new_events))
        # Add maintenance type to transaction details (optional)
        if maintenanceType:
            self.transactions[-1] = self.transactions[-1] + ("maintenanceType", None, maintenanceType)

    def str(self):
        return f"Park(id={self.id}, name={self.name}, objType={self.objType}, adress={self.adress}, capacity={self.capacity}, owner={self.owner}, openingDate={self.openingDate}, greenArea={self.greenArea})"
    ...
    def set_attribute(self, attribute_name, value):
        if attribute_name == "greenArea":
            if not isinstance(value, int) or value < 0:
                raise ValueError("Value of greenArea must be a non-negative integer.")
            self.greenArea = value
        else:
...
class Library(CityObject):
  def __init__(self, name=None, objType="Library", capacity=0, owner=None, openingDate=None, bookCount=0, **kwargs):
    super().__init__(name, objType, capacity, owner, openingDate, **kwargs)
    self.bookCount = bookCount  # Новый атрибут данных, специфичный для библиотек

  def addBook(self, bookTitle, author, isbn, quantity=1):
    # Новый метод, специфичный для библиотек
    old_bookCount = self.bookCount
    self.bookCount += quantity
    self.transactions.append((datetime.utcnow(), "addBook", "bookCount", old_bookCount, self.bookCount))

  def __str__(self):
    return f"Library(id={self.id}, name={self.name}, objType={self.objType}, adress={self.adress}, capacity={self.capacity}, owner={self.owner}, openingDate={self.openingDate}, bookCount={self.bookCount})"
...
city_object = Park(name="Museum", objType="Cultural", address="123 Main St", capacity=100, owner="ABC Corp", openingDate="2022-01-01")

# Получение значения greenArea для объекта Park
if isinstance(city_object, Park):
  green_area = city_object.get_attribute("greenArea")
  print(f"Площадь парка: {green_area}")
else:
    print("NO")

# Установка значения bookCount для объекта Library
if isinstance(city_object, Library):
  city_object.set_attribute("bookCount", 1000)
else:
    print("No")
